...
class Vtiger_MassActionAjax_View extends Vtiger_IndexAjax_View {
	...
	function showSendSMSForm(Vtiger_Request $request) {
        ...
        $moduleModel = Vtiger_Module_Model::getInstance($sourceModule);
		$SMSActiveModule = Vtiger_Module::getInstance($moduleName);
		...
		$senderID = SMSNotifier::getSMSSender();
		$SMSLanguage = SMSNotifier::getSMSLanguage();
		$SMSTemplate = SMSNotifier::getSMSTemplate();
		...
		$viewer->assign('SMSTemplate', $SMSTemplate);
		$viewer->assign('EXCLUDED_IDS', $excludedIds);
