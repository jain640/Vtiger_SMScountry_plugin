	...
	public function getLinks() {
		...
		if (array_key_exists($relatedModuleName, $relationModel->getEmailEnabledModulesInfoForDetailView())) {
			...
			if ($currentUserPriviligesModel->hasModulePermission(getTabid('Emails'))) {
                ...
                if($relatedModuleName=='Leads'||$relatedModuleName=='Contacts'||$relatedModuleName=='Accounts'){
                  $emailLink = Vtiger_Link_Model::getInstanceFromValues(array(
                                'linktype' => 'LISTVIEWBASIC',
                                'linklabel' => 'SendSms',
                                'linkurl' => "javascript:Vtiger_List_Js.triggerSendSms('index.php?module=$relatedModuleName&view=MassActionAjax&mode=showSendSMSForm&category=Campaign','SMSNotifier');",
                                //'linkurl' => "javascript:Campaigns_RelatedList_Js.triggerSendSms('index.php?module=$relatedModuleName&view=MassActionAjax&mode=showSendSMSForm,'SMSNotifier');",
                                'linkicon' => ''
                        ));
                        //      $massActionLink->set('_sendSMS',true);
                                $emailLink->set('_sendSms',true);
                                $relatedLinks['LISTVIEWBASIC'][] = $emailLink;
}
