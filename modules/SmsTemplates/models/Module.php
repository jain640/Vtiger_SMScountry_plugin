...
class SmsTemplates_Module_Model extends Vtiger_Module_Model {
	 ...
	 * @param SmstTemplates_Record_Model $recordModel
	...
	public function saveRecord($recordModel) {
		...
		if(empty($templateid)){
			$result = $db->pquery('SELECT count(*) AS count FROM vtiger_smstemplates', array());
			$templateid = 1 + ((int)$db->query_result($result, 0, 'count'));
			$result = $db->pquery('SELECT count(*) AS count FROM vtiger_smstemplates_seq', array());
			if($db->query_result($result, 0, 'count')==0)
				$db->pquery('insert into vtiger_smstemplates_seq SET id = ?', array($templateid));
			else
				$db->pquery('UPDATE vtiger_smstemplates_seq SET id = ?', array($templateid));
			$sql = "INSERT INTO vtiger_smstemplates(templatename, message, deleted, templatelanguage, templateid) VALUES (?,?,?,?,?)";
		}else{
			$sql = "UPDATE vtiger_smstemplates SET templatename=?, message=?, deleted=?, templatelanguage=? WHERE templateid = ?";
		}
		$params = array(decode_html($recordModel->get('templatename')), $recordModel->get('message'), 0, $recordModel->get('templatelanguage'), $templateid);
		$db->pquery($sql, $params);
		return $recordModel->setId($templateid);
	}

	/**
	 * Function to delete the sms template
	 * @param type $recordIds
	 */
	public function deleteRecord($recordModel) {
		$recordId = $recordModel->getId();
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM vtiger_smstemplates WHERE templateid = ? ', array($recordId));
	}
	
	/**
	 * Function to delete all the sms templates
	 * @param type $recordIds
	 */
	public function deleteAllRecords() {
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM vtiger_smstemplates', array());
	}

	/**
	 * Function to get Sms template fields from modules
	 * @return <array> template fields
	 */
	public function getAllModuleSmsTemplateFields() {
	...
	/**
	 * Function to get module list which has the sms field.
	 * @return type
	 */
	...
	public function getRecordIds($skipRecords){
		...
		$query = 'SELECT templateid FROM vtiger_smstemplates WHERE templateid NOT IN ('.generateQuestionMarks($skipRecords).')';
