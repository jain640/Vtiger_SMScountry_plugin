                        ...
                        {foreach item=RELATED_LINK from=$RELATED_LIST_LINKS['LISTVIEWBASIC']}
                            <div class="btn-group">
                                ...
                                {assign var=IS_SEND_EMAIL_BUTTON value={$RELATED_LINK->get('_sendEmail')}}
                                 {assign var=IS_SEND_SMS_BUTTON value={$RELATED_LINK->get('_sendSms')}}
                                <button type="button" class="btn addButton
                                {if $IS_SELECT_BUTTON eq true} selectRelation {/if} "
                            {if $IS_SELECT_BUTTON eq true} data-moduleName='{$RELATED_LINK->get('_module')->get('name')}' {/if}
                        {if $RELATION_FIELD} data-name="{$RELATION_FIELD->getName()}" {/if}
                {if $IS_SEND_EMAIL_BUTTON eq true or $IS_SEND_SMS_BUTTON eq true}	onclick="{$RELATED_LINK->getUrl()}" {else} data-url="{$RELATED_LINK->getUrl()}"{/if}
                {if ($IS_SELECT_BUTTON eq false) and ($IS_SEND_EMAIL_BUTTON eq false) and ($IS_SEND_SMS_BUTTON eq false)}
