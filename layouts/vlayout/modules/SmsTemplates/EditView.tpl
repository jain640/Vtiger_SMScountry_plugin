...
<div class='editViewContainer'>
	<form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php">
		...
		<table class="table table-bordered blockContainer showInlineTable">
			<tr>
				<th class="blockHeader" colspan="4">{vtranslate('SINGLE_SmsTemplates', $MODULE)}</th>
			</tr>
			<tr>
			...
			<tr>
				<td class="fieldLabel {$WIDTHTYPE}">{vtranslate('LBL_TEMPLATE_LANGUAGE', $MODULE)}</td>
				<td class="fieldValue {$WIDTHTYPE}">
					<span class="span4 conditionRow">
						<select name="templatelanguage" id="templatelanguage" data-placeholder="{vtranslate('LBL_FROM_FIELDS',$MODULE)}" class="chzn-select">
							{foreach item=post_value key=post_key from=$SMSLanguage}
								<option value="{$post_key}" {if $RECORD->get('templatelanguage') eq $post_key} selected {/if}>{$post_value}</option>
							{/foreach}
		...
		<table class="table table-bordered blockContainer showInlineTable">
			<tr>
				<th class="blockHeader" colspan="4">{vtranslate('LBL_SMS_TEMPLATE', $MODULE)} {vtranslate('LBL_DESCRIPTION', $MODULE)}</th>
			</tr>
			<tr>
		...
		<div class="row-fluid padding-bottom1per">
			<textarea id="templatecontent" name="message">{$RECORD->get('message')}</textarea>
		</div>
		<div id='counter'>
